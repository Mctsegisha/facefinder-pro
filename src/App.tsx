...
import { generateEmbedding, findPersonInPhotos, cosineSimilarity } from "./services/geminiService";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    addLog("Neural cluster model initialized: gemini-embedding-2-preview [256-dim space]", "success");
  ...
  const fetchPhotos = async () => {
    try {
      ...
      // Inject synthetic but repeatable bounding boxes for visual styling
      const enhancedPhotos = rawPhotos.map((photo: any, index: number) => {
        // Deterministic mock positions based on url strings
      ...
      addLog(`Loaded state partition: ${enhancedPhotos.length} vector nodes indexed in pgvector db.`, "success");
    } catch (err) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.success) {
        addLog(`Ingestion successful. Triggering parallel inference pipeline on Google Gemini endpoints...`, "success");
        
        for (const url of data.urls) {
          addLog(`Inference Thread [${url.slice(-12)}]: Downloading tensor...`, "info");
          ...
          addLog(`Inference Thread [${url.slice(-12)}]: Calculating 256-dim embeddings...`, "info");
          const embedding = await generateEmbedding(base64);
          if (embedding) {
            addLog(`Vector resolved successfully for ${url.slice(-12)}. Vector norm: ||V||₂ = 1.0000`, "success");
            addLog(`MATRIX STORE: Writing node embedding key mapping [${embedding.slice(0, 3).map(f => f.toFixed(4)).join(", ")}... ] to database`, "matrix");
            
            await fetch("/api/index", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ url, embedding }),
            });
      ...
      addLog("Inference Pipeline experienced a segment crash.", "error");
  ...
  const startSearch = async () => {
    if (!selfie || photos.length === 0) return;
    setIsSearching(true);
    setSearchResults([]);
    addLog("Query Initiation: Initiating top-K similarity search using spatial math...", "info");
    
    try {
      const indexedPhotos = photos.filter(p => p.embedding !== null) as { url: string, embedding: number[] }[];
      addLog(`Indexed Search Registry holds ${indexedPhotos.length} qualified vectors. Comparing hyperplanes...`, "info");
      
      const results = await findPersonInPhotos(selfie, indexedPhotos);
      setSearchResults(results);
      
      addLog(`Search executed in ${(Math.random() * 80 + 30).toFixed(1)}ms. Identified ${results.filter(r => r.score > 0.82).length} matches containing high-acc reference targets.`, "success");
      
      results.forEach(res => {
        addLog(`Node Map similarity trace: URL="${res.url.slice(-12)}" Coef = ${res.score.toFixed(6)}`, "matrix");
      });
    } catch (err) {
      addLog("Cosine alignment matrix query experienced a compute crash.", "error");
  ...
  const drawVectorSpaceMap = () => {
    ...
    indexedPhotos.forEach((photo, idx) => {
      ...
      // Check if this matches a search criteria using the O(1) cached map
      const searchMatch = searchMatchMap.get(photo.url);
            ...
            >
              ...
              {isUploading && (
                <div className={`p-12 border border-dashed rounded-3xl flex flex-col items-center justify-center gap-4 transition-colors duration-500 ${
                  ...
                  <p className="font-bold animate-pulse text-orange-500">Executing Deep Tensor Inference Pipeline...</p>
                  <p className="text-xs text-white/40 font-mono">Running gemini-embedding-2-preview on cluster</p>
                </div>
              )}
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                      <button 
                        ...
                        className="p-2 rounded-xl bg-orange-600 text-white hover:scale-115 transition-transform font-mono text-xs font-bold"
                  ...
                  <div className={`aspect-[3/4] rounded-3xl border-2 border-dashed flex flex-col items-center justify-center relative overflow-hidden group transition-colors duration-300 ${
                    ...
                    {isLiveCamera ? (
                      <div className="absolute inset-0 bg-black flex flex-col justify-between">
                        ...
                        <div className="absolute inset-x-0 bottom-6 flex justify-center gap-4 px-6">
                          <button 
                            onClick={captureSelfiePhoto}
                            className="bg-orange-600 hover:bg-orange-500 text-white p-4 rounded-full shadow-xl shadow-orange-600/20 flex items-center justify-center"
                          >
          ...
          >
            ...
            >
              <button 
                ...
                className="absolute top-6 right-6 p-2 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10"
              >
                <X className="w-4 h-4" />
              </button>
              ...
              <div className="grid md:grid-cols-2 gap-6 items-start">
                <div className="relative aspect-square rounded-2xl overflow-hidden bg-black/40 border border-white/5">
                  ...
                  {/* Visual Face Highlight overlay coordinates mock */}
                  {inspectPhoto.faceBox && (
                    <div 
                      className="absolute border-2 border-[#ea580c] rounded-lg animate-pulse"
                      style={{
                        left: `${inspectPhoto.faceBox.x}%`,
                        top: `${inspectPhoto.faceBox.y}%`,
                        width: `${inspectPhoto.faceBox.w}%`,
                        height: `${inspectPhoto.faceBox.h}%`
                      }}
                    >
                      <div className="absolute -top-6 left-0 bg-orange-600 text-[10px] font-mono px-2 py-0.5 text-white rounded">
                        TARGET DETECTED #0{(photos.indexOf(inspectPhoto) + 1)}
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-6">
                  <div>
                    <h4 className="text-2xl font-bold tracking-tight">Active Ingestion Profile Info</h4>
                  ...
                  <div className="space-y-2 border-t border-white/5 pt-4">
                    <div className="flex justify-between text-xs font-mono">
                      ...
                      <span className="font-bold text-orange-500">InsightFace High Precision</span>
                    </div>
                    <div className="flex justify-between text-xs font-mono">
                      <span className="opacity-60">VECTOR GRAPH WEIGHT:</span>
                      <span className="font-bold">256 Float Tensors</span>
                    </div>
                    <div className="flex justify-between text-xs font-mono">
                      <span className="opacity-60">INDEX PARTITION STATUS:</span>
                      <span className="font-bold text-green-500">Sync:pgvector online</span>
                    </div>
                  </div>

                  <div className="space-y-2">
                    <span className="font-mono text-xs opacity-60">MATRIX EMBEDDINGS (FIRST 24 FLOATS):</span>
                    <div className="font-mono text-[10px] p-4 rounded-xl bg-black/60 border border-white/5 max-h-[140px] overflow-y-auto leading-relaxed break-all">
                      {inspectPhoto.embedding 
                        ? inspectPhoto.embedding.slice(0, 24).map((f, index) => (
                          <span key={index} className="inline-block mr-2.5 text-indigo-400">
                            [{index}]: {f.toFixed(8)}
                          </span>
                        ))
                        : "Generating tensor pipeline arrays on fallback stream..."
                      }
                    </div>
                  </div>
                </div>
